...
export function PipelineCard({ lead }: { lead: any }) {
  const {
    setNodeRef,
    attributes,
    listeners,
    ...
    <div ref={setNodeRef} style={style} {...attributes} {...listeners} className="touch-none cursor-grab">
      <Card className="hover:border-blue-500/50 transition-colors shadow-sm bg-white dark:bg-neutral-900 border-neutral-200 dark:border-neutral-800">
        <CardHeader className="p-3 pb-2 flex flex-row justify-between items-start">
          ...
          <Badge variant={lead.leadScore > 80 ? 'default' : 'secondary'} className="text-[10px] px-1 py-0 ml-2">
            {lead.leadScore}
          </Badge>
        </CardHeader>
        <CardContent className="p-3 pt-0 flex flex-col gap-2">
          <div className="flex items-center gap-2 text-xs text-neutral-500">
            ...
            <span className="line-clamp-1">{lead.neighborhood || 'Não informado'}</span>
          </div>
          
          <div className="flex flex-wrap gap-1 mt-1">
            <Badge variant="outline" className="text-[10px] px-1 py-0">{lead.category}</Badge>
